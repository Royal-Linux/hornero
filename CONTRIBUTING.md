...
1. Fork https://github.com/Royal-Linux/hornero using GitHub's interface to your own account.
   Let's say that the forked repository is at
   `https://github.com/YOUR_GITHUB_USERNAME/hornero` .
2. Clone the main hornero repository https://github.com/Royal-Linux/hornero to a local folder on
   your computer, say named hornero/ (`git clone https://github.com/Royal-Linux/hornero hornero`)
3. `cd hornero`
4. `git remote add pullrequest https://github.com/YOUR_GITHUB_USERNAME/hornero`
...
7. On GitHub's web interface, go to: https://github.com/Royal-Linux/hornero/pulls
...
9. If there are merge conflicts, or a branch lags too much behind hornero's master,
...
The point of doing the above steps, is to never directly push to the main hornero
repository, _only to your own fork_. Since your local `master` branch tracks the
main hornero repository's master, then `git checkout master`, as well as
...
1. `hub clone Royal-Linux/hornero my_hornero`
2. `cd my_hornero`
...
### Improve hornero by making commits:
...
(so that your changes can be merged to the main hornero repository)
